...
def _load_models_background():
    ...
    Thread-safety note: _tokenizer, _llm, _tts are written ONCE here and then
    only READ by async request handlers. With --workers=1 (single process) and
    --concurrency=1, no race condition exists.
    """
...
async def tts_endpoint(req: HaryanviIn):
    ...
    loop = asyncio.get_running_loop()
    await loop.run_in_executor(None, synthesize, _tts, req.text, str(path))
    return FileResponse(path, media_type="audio/wav", filename="haryanvi_tts.wav")
