...
- Stage 1 — Text-to-Text Translation: A Hindi → Haryanvi (Bangru dialect) neural machine translation model.
- Stage 2 — Text-to-Speech Synthesis: A Haryanvi TTS model that generates natural-sounding audio from Haryanvi text.
This milestone documents the datasets collected and created for both stages, including their sources, quality assessment, preprocessing steps, and train/validation/test split strategies.
   ...
   ## 1.1  Team Member's contribution

   - **Abhishek** (22f3000978@ds.study.iitm.ac.in)
      1. ALL further preprocessing, 
      2. For LLM fine-tuning tasks, dataset formatting (e.g., instruction-response structure), token length considerations, and prompt structuring requirements.

   - **Satyam Srivastava** (21f1000629@ds.study.iitm.ac.in)
      1. Make a structured folder for both audio and text dataset and upload in hugging face as a project dataset.
      2. Extracted 3 audio files from youtube using 'yt-dlp' library.
      3. Scraped Haryanvi-Hindi parallel dataset from [storyweaver_site](https://storyweaver.org.in)

   - **Sanket Agrawal** (23f1001709@ds.study.iitm.ac.in)
      1. Written a python script for synthetic text data-generation and augmentation.
      2. Documented a report on this synthetic data-generation.

   - **Md Fazlur Rahman** (23f1001897@ds.study.iitm.ac.in)
      1. Scraped text data from google-reasearch-datasets/indic-gen-bench.
      2. Maked milestone-2-report.
...
# Module A: Text Dataset (Hindi → Haryanvi Translation)**
## 3.1 Data Sources

Two primary sources were used to build the parallel Hindi–Haryanvi corpus:
### Source 1: XOR-QA Indic Dataset (Google Research)
URL: `https://github.com/google-research-datasets/indic-gen-bench/blob/main/xorqa\_in/xorqa\_bgc\_dev.json`
Format: JSON
...
Format: CSV and JSON (`synthetic\_haryanvi\_corpus.json`)
Size: 5,000 unique parallel sentence pairs
Generation Approach: A research-grade prompt was used to enforce strict linguistic rules, topic diversity, and structural variety. See the generation report for full details.
...
Citation: Storyweaver (Pratham Books). Digital collection accessible at [Storyweaver](https://storyweaver.org.in)
Status: Not used in the current pipeline. Identified as a data expansion fallback if the primary corpus (Sources 1 + 2) proves insufficient for model convergence.
Rationale for deferral: Extracting structured parallel pairs from the digitized book requires non-trivial parsing effort. This will be pursued if and when the need arises.
Usage Constraints: Open-source content licensed under Creative Commons; requires attribution to Pratham Books.
...
Short (5–8 words)	20%
Medium (10–15 words)	25%
Long (20+ words)	20%
Two-sentence	15%
Three-sentence	10%
Paragraph (5–6 sentences)	10%
...
Duplicates: Exact duplicate rows were removed using `drop\_duplicates()`. The count of removed rows was logged for traceability.
...
Splitting was performed using `sklearn.model\_selection.train\_test\_split` with a fixed random seed (`RANDOM\_SEED = 42`) to ensure reproducibility.
...
Access: Loaded programmatically via `load\_dataset("ankitdhiman/haryanvi-tts")` using the HuggingFace `datasets` library
...
Extraction Tool: `yt-dlp` — audio downloaded in best available quality.**
Transcription Strategy (two-tier):
Primary: `youtube-transcript-api` attempts to fetch the YouTube-native transcript using the video ID parsed from the URL. If found, it is used directly.
Fallback: If no transcript is available, OpenAI Whisper (`tiny` model) is run on the extracted audio to generate timestamped segments.
Output Format: Timestamped transcript saved as a CSV-like `.txt` file with entries in the format `\[MM:SS.mmm --> MM:SS.mmm] text`, one segment per row (`formatted\_transcript.txt`)
...
Convert MP3 → WAV at 22,050 Hz mono (to match `TARGET\_SR` in `DataConfig`)
Segment audio using Whisper timestamp boundaries into individual clips
Manually verify a random sample of Whisper transcripts for dialect accuracy
Log video ID alongside each clip for speaker-stratified splitting
Apply the same duration filtering (1.0–10.0s) and deduplication as Source 1
> \*\*Note on Whisper model size:\*\* The pipeline currently uses `whisper tiny` for speed. If the fallback is activated, upgrading to `whisper medium` or `whisper large-v3` is strongly recommended for significantly better transcription accuracy on Haryanvi audio.
...
All audio was resampled to 22,050 Hz using the `datasets.Audio(sampling\_rate=22050)` cast, which applies consistent resampling regardless of the original source format.
...
Audio splits were created using HuggingFace's `train\_test\_split()` method:
...
The same fixed seed (`RANDOM\_SEED = 42`) was used to ensure reproducibility across runs.
Data Leakage Prevention:
...
Column	Description
`audio\_array`	Raw audio waveform as a NumPy array at 22,050 Hz
`normalized\_text`	Whitespace-stripped Haryanvi transcript
...
If model performance (e.g., BLEU score, manual evaluation) is unsatisfactory after initial fine-tuning, the next expansion step is parsing bilingual passages from Grierson's Linguistic Survey of India — a high-authenticity source held in reserve to avoid premature engineering overhead.
...
Activate the YouTube fallback — extract and verify audio from the three identified Haryanvi video sources using the `youtube\_video\_to\_audio.py` pipeline.
Transfer learning — initialize from a pre-trained Hindi TTS checkpoint (e.g., IndicTTS Hindi or Sarvam-TTS) and fine-tune on available Haryanvi audio, leveraging the strong phonological and script-level proximity between Hindi and Haryanvi.
...
Grierson (Book)	Public domain (1916); no copyright constraints; retained as fallback only
...
All preprocessing steps are implemented across two Google Colab notebooks: `rds.py` (main data pipeline) and `youtube\_video\_to\_audio.py` (fallback audio extraction).
[x] Fixed random seed (`RANDOM\_SEED = 42`) used for all splits
...
Parallel Hindi–Haryanvi sentence pairs
...
Current corpus: 5,594 pairs — if model performance is unsatisfactory, Grierson parsing will be pursued as the next expansion step
